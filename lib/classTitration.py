...
class Titration(object) :
    ...
    def requiredVol(self, Cacid, pHf) :
        ...
        pK1c = 3670.7 / self.T - 62.008 + 9.7944 * np.log(self.T) - 0.0118 * self.S + 0.000116 *(self.S ** 2) # H2CO3  --> H + HCO3 (Mehrbach 1973 refitted by Dickson and Millero 1987)
        pK2c = 1394.7 / self.T + 4.777 - 0.0184*self.S + 0.000118 * (self.S ** 2) # HCO3   --> H + CO3  (Mehrbach 1973 refitted by Dickson and Millero 1987F)
        lnKw = 148.96502 - 13847.26 / self.T - 23.6521 * np.log(self.T) \
               + (118.67 / self.T - 5.977 + 1.0495 * np.log(self.T)) * (self.S ** 0.5) - 0.01615 * self.S  # (DOE 1994)
        lnKB = (-8966.90 - 2890.53 * (self.S ** 0.5) - 77.942 * self.S + 1.728 * (self.S ** 1.5) - 0.0996 * (self.S ** 2)) / self.T \
        ...
        denumCinit = np.square(np.power(10, -1 * self.pHs[-1])) + K1c * np.power(10, -1 * self.pHs[-1]) + K1c * K2c
        denumCf = np.square(np.power(10, -1 * pHf)) + K1c * np.power(10, -1 * pHf) + K1c * K2c
        ...
        acidVol = self.sampleSize / Cacid * (np.power(10, -1 * pHf) - np.power(10, -1 * self.pHs[-1]) - (C_alk_f - C_alk_i) \
                                    - (BAf - BAi) - Kw * (np.reciprocal(np.power(10, -1 * pHf)) \
