...
class App(tk.Tk) :
    ...
    def __init__(self) :
        ...
        self.StartButton = tk.Button(self, text = 'Start', bg = 'green', padx = 20, command = self.start)
        ExitButton = tk.Button(self, text = 'Exit', bg = 'red', padx = 20, command = self.quitProgram)
        ...
        self.canvas.get_tk_widget().grid(row = 0, column = 2, rowspan = 4, columnspan = 3, sticky = 'NSEW')
        self.canvas.draw()
    ...
    def start(self):
        ...
        titration = classTitration.Titration(sampleSize, salinity, temp, np.array([pHi]), np.array([emfi]), np.array([0]))
        
        self.StartedLabel = tk.Label(self, text = "Titration Started!", fg = 'green', pady = 10)
        self.StartedLabel.grid(row = 5, column = 0)
        self.stopTitration = False

        self.StopButton = tk.Button(self, text = 'Stop Titration', padx = 10, command = self.cancelTitration)
    ...
    def InitialTitration(self, titration) :
        ...
        if (targetPos <= 0) :
            ...
            titration.volumeAdded = np.append(titration.volumeAdded, titration.volumeAdded[-1] + syringeStep * self.syringePos )
            ...
            titration.pHs = np.append(titration.pHs, pH)
            titration.emf = np.append(titration.emf, emf)
            titration.volumeAdded = np.append(titration.volumeAdded, titration.volumeAdded[-1] + syringeStep * numSteps)
    ...
    def writeData(self, titration, TA) :
        ...
        with open('data/' + filename + '.csv', 'w') as f:
            ...
            writer.writerow(['Volume Added:'] + [','.join(titration.volumeAdded.astype(str))])
